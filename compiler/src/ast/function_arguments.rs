...
use super::{
    new_err, r#type::{IntoType, TypecheckFlags}, Dependencies, Dependency, FunctionParameters, TypeLayout, Value,
};
...
impl Parser {
    pub fn function_arguments(
        ...
        for (idx, child) in children.enumerate() {
            ...
            if !expected_ty_at_idx.eq_complex(user_gave, &TypecheckFlags::use_class(maybe_class_type).lhs_unwrap(false)) {
