...
use super::{r#type::IntoType, Assignment, Number, Value, INT_TYPE, list::ListBound};
...
impl Parser {
    pub fn assignment_unpack(
		...
		if is_modify {
			return Err(vec![new_err(input_span, file_name, "assignment unpacking does not support the modify keyword".to_owned())])
		}
        ...
        Ok((Assignment::new_multi(idents.into_boxed_slice(), value), false))
    }
}
